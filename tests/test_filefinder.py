...
def __default_err_msg(extension: str = None) -> str:
    return f"extension '{extension}' must be in {FileFinder.EXTENSION_CHOICES}"
...
def test_file_finder_wrong_setup(temp_tree_structure1):
    ...
    with pytest.raises(AssertionError) as err:
        FileFinder(single_start_dir=temp_tree_structure1, multi_start_dir=temp_tree_structure1, extension=".txt")
...
def test_filefinder_caw_histtags(temp_tree_structure2):
    file_finder = FileFinder(
        single_start_dir=temp_tree_structure2, extension=".csv", filename_regex="^[0-9]{8}_HistTags$", limit_depth=False
    )
