...
def test_file_finder_no_depth_limit(temp_tree_structure1):
    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".txt", limit_depth=False)
    assert len(file_finder.paths) == 3

    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".jpg", limit_depth=False)
    assert len(file_finder.paths) == 2

    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".png", limit_depth=False)
...
def test_file_finder_depth_0(temp_tree_structure1):
    ...
    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".txt", limit_depth=True, depth=0,)
    assert len(file_finder.paths) == 1
    assert file_finder.paths[0].name == "maindir_file1.txt"

    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".jpg", limit_depth=True, depth=0,)
    assert len(file_finder.paths) == 0


def test_file_finder_depth_1(temp_tree_structure1):
    """ Limit the search to single_start_dir and its subdirs"""
    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".txt", limit_depth=True, depth=1,)
    assert len(file_finder.paths) == 2
    ...
    file_finder = FileFinder(single_start_dir=temp_tree_structure1, extension=".jpg", limit_depth=True, depth=1,)
    assert len(file_finder.paths) == 2
    assert sorted([x.name for x in file_finder.paths]) == [
        "subdir1_file2.jpg",
        "subdir2_file1.jpg",
...
def test_file_finder_multi_start_dir(temp_tree_structure1):
    ...
    file_finder = FileFinder(multi_start_dir=[dir_path1, dir_path2], extension=".txt", depth=1, limit_depth=True,)
...
def test_file_finder_wrong_setup(temp_tree_structure1):
    ...
    with pytest.raises(AssertionError) as err:
        FileFinder(
            single_start_dir=temp_tree_structure1, extension=".txt", limit_depth=False, depth=4,
        )
    ...
    with pytest.raises(AssertionError) as err:
        FileFinder(
            single_start_dir=temp_tree_structure1, extension=".txt", filename_regex=re.compile("abc"),
        )
    ...
    with pytest.raises(AssertionError) as err:
        FileFinder(
            single_start_dir=temp_tree_structure1, multi_start_dir=temp_tree_structure1, extension=".txt",
        )
